...
class ConversationController extends Controller
{
    ...
    public function chatCutOff(Request $request)
    {
        $convo=Conversation::where('id',$request->id)->first();
        if($convo->status=='paid'){
            return response()->json(['user' => 'Already Paid', 'code' => 201]);
        }
        $user = User::where('id', auth()->user()->id)->value('profileprogess_id');
        $profile = ProfileProgress::where('id', $user)->first();
        if ($profile->types_points < 150) {
            return response()->json(['user' => 'Points Not Enough', 'code' => 269]);
        }
        $profile->types_points -= 150;
        $profile->save();
        $convo->status='paid';
        $convo->save();
        return response()->json(['user' => 'Points Cut Off Successfully','code' => 200]);
    }
