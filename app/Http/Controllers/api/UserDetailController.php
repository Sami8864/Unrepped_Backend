...
class UserDetailController extends Controller
{
    ...
    public function getUserMediaDevice(int $id)
    {
    ...
    public function filmmakers()
    {
        ...
        foreach ($users as $user) {
            if ($user->user_type === 'filmmaker') {
                $device['user'] = $user;
                $device['progress'] = FilmMaker::where('user_id', $user->id)->first();
    ...
    public function performers()
    {
        ...
        foreach ($users as $user) {
            if ($user->user_type === 'user') {
                ...
                $device['media'] =  $this->getUserMediaDevice($user->profileprogess_id);
                $performers[] = $device;
            }
        };
        return response()->json(['message' => 'Performers fetched successfully', 'code' => 200, 'data' => $performers], 200);
    ...
    public function getTransactions()
    {
        ...
        $transactions = UserSpending::all();

        // Initialize an empty array to store transactions with user details
        $transactionsWithUserDetails = [];

        // Iterate over each transaction
        foreach ($transactions as $transaction) {
            // Find the corresponding user from the 'users' table
            $user = User::find($transaction->user);

            // If user is found, add transaction details with user's name and email to the new array
            if ($user) {
                $transactionData = [
